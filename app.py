...
# in app imports
from backend.groupme import fetchGroupData, getMessages, formatMessages
from backend.one_ai import oneAi_summary
from backend.mongoDb import insert_into_mongodb, retrieve_from_mongodb


load_dotenv(override=True) # load the keys from local env.
...
# database configuration--------------------------------------------------------
uri = F"mongodb+srv://{MONGO_USERNAME}:{MONGO_PW}@cluster0.vfj6kgs.mongodb.net/?retryWrites=true&w=majority"
>>>>>>> ab5d83a (file organiazon)
...
=======
    messages_day = formatMessages(retrieve_from_mongodb(messages_collection, group_id=str(group_id), before_time=one_day_ago))
    messages_week = formatMessages(retrieve_from_mongodb(messages_collection, group_id=str(group_id), before_time=one_week_ago))
    print(messages_day)
    print(messages_week)
    print("A")
    summary_day = oneAi_summary(ONEAI_KEY, messages_day, "summarize")
    summary_week = oneAi_summary(ONEAI_KEY, messages_week, "summarize")
>>>>>>> ab5d83a (file organiazon)
